	...
	// 图书转移（2011-12-1）�-------------------------------------------------
	function importTransInit() {
		pag.showAdd(baseUrl + '?action=importTransInit', 'book_update',
				'book_index', function() {
				});
	}
...
//		}
	...
	function downloadTrans() {
		window.location.href = baseUrl + '?action=downloadTrans';
	}
	...
	function importHandle()
	{
	   ...
	   if (!et || et.value.strip() == '') {
		...
		// 导入图书迁移操作--------------------
	function importHandleTrans(){
		
		var et = $('importCityId');
	    if (!et || et.value.strip() == ''||et.value.strip()=='-1') {
		 	base.alert('请选择图书所属镇区',function(){et.focus();});
		 	return false;
	 	}
	    et = $('importAgencyId');
	    if (!et || et.value.strip() == ''||et.value.strip()=='-1') {
		 	base.alert('请选择图书所属机构',function(){et.focus();});
		 	return false;
	 	}
	 	et = $('importSchoolId');
	    if (!et || et.value.strip() == ''||et.value.strip()=='-1') {
		 	base.alert('请选择图书所属学校',function(){et.focus();});
		 	return false;
	 	}
	    
	    var et = $('descCityId');
	    if (!et || et.value.strip() == ''||et.value.strip()=='-1') {
		 	base.alert('请选择迁移目标镇区',function(){et.focus();});
		 	return false;
	 	}
	    et = $('descAgencyId');
	    if (!et || et.value.strip() == ''||et.value.strip()=='-1') {
		 	base.alert('请选择迁移目标机构',function(){et.focus();});
		 	return false;
	 	}
	 	et = $('descSchoolId');
	    if (!et || et.value.strip() == ''||et.value.strip()=='-1') {
		 	base.alert('请选择迁移目标学校',function(){et.focus();});
		 	return false;
	 	}	    
...
//	 	base.confirm('您确信要进行图书迁移操作吗？', function() {
//			alert("dfd");
	 		return true;
//		})
	 		return false; 			
	...
	function toCategory(id,nameId,codeId){
		new Ajax.Request(
			'/library/book.do?action=getCategoryNameCode&id='+id,
				{
					method : 'POST',
					evalScripts : true,
					parameters : '',
