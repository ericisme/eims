 ...
 * @version 1.0
 */
public class BookJsysController extends SimpleManageController<Book, BookService> {
	...
	String importPage;
		...
		final static Integer currentSjlx = BookService.SJLX_JSYS; //设置 当前书籍类型 = 老师用书	
	
	public ModelAndView index(HttpServletRequest request, HttpServletResponse response) throws Exception {
		Map<String, Object> map = this.getMapWithUser(request);
		User user = (User) map.get("user");
		boolean all = PriviilegeFilter.isAllowd(user, "/library/bookJsys.do", "all");
		//本市
		if(all){
		//本镇区	
		}else if (PriviilegeFilter.isAllowd(user, "/library/bookJsys.do", "area")){
...
//			querySchoolId = user.getSchool_id();
	...
	public ModelAndView importBookSubmit(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if(errorList.size()>0){
			...
			return new ModelAndView("/library/bookJsys/book_error_list", map);//显示错误提示页面
		...
		msg = "<script>alert('数据导入成功！');window.location.href=\"/library/bookJsys.do?action=index\";</script>";
		ServletOutputStream out = response.getOutputStream();
		out.print(msg);
		out.close();
		return  null;
